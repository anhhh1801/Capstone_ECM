...
import { UserPlus, Mail, Phone, User } from 'lucide-react-native';
            ...
            >
                <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', padding: 24 }}>
                    ...
                    <View className="bg-white p-6 rounded-2xl shadow-sm space-y-4">
                        ...
                        <View className="flex-row gap-3">
                            <View className="flex-1">
                                <View className="flex-row items-center border border-gray-300 rounded-lg px-3 bg-gray-50">
                                    ...
                                    <TextInput
                                        className="flex-1 p-3 text-gray-900 ml-1"
                                        placeholder="Last name (Tran)"
                        ...
                        <View className="flex-row items-center border border-gray-300 rounded-lg px-3 bg-gray-50 mt-5">
                            <Mail size={18} color="#9ca3af" />
                            <TextInput
                                className="flex-1 p-3 text-gray-900 ml-2"
                        ...
                        <TouchableOpacity
                            ...
                            disabled={loading}
                            className={`mt-4 w-full py-3.5 rounded-xl flex-row justify-center items-center ${loading ? 'bg-indigo-400' : 'bg-indigo-600'}`}
                        >
                            {loading ? (
                                <ActivityIndicator color="white" />
                            ) : (
                                <Text className="text-white font-bold text-lg">Sign Up Now</Text>
                            )}
                        </TouchableOpacity>
                    ...
                    <View className="flex-row justify-center mt-6">
                        ...
                        <TouchableOpacity onPress={() => navigation.navigate("Login")}>
                            <Text className="font-bold text-indigo-600">Login Now</Text>
                        </TouchableOpacity>
                    </View>
