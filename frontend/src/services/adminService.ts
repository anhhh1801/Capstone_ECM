...
export const toggleUserLock = async (adminId: number, targetUserId: number) => {
    ...
    const response = await api.put(`/users/admin/lock?adminId=${adminId}&targetUserId=${targetUserId}`);
    return response.data;
};

// 3. Get User Stats
export const getUserStats = async (adminId: number, targetUserId: number) => {
    const response = await api.get(`/users/admin/stats?adminId=${adminId}&targetUserId=${targetUserId}`);
    return response.data;
};