"use client";

import Link from "next/link";
import { AlertTriangle, Home } from "lucide-react";
import { useEffect, useState } from "react";

interface ErrorProps {
    error: Error & { digest?: string };
    reset: () => void;
}

export default function Error({ error, reset }: ErrorProps) {
    const [isMounted, setIsMounted] = useState(false);

    useEffect(() => {
        setIsMounted(true);
        console.error("Error caught:", error);
    }, [error]);

    if (!isMounted) {
        return null;
    }

    return (
        <div className="flex-1 flex items-center justify-center bg-gradient-to-br from-[var(--color-soft-white)] to-[var(--color-main)]/30 px-4">
            <div className="w-full max-w-md rounded-2xl bg-[var(--color-secondary)]/40 p-8 shadow-xl text-center">
                {/* Icon */}
                <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-[var(--color-alert)]/20">
                    <AlertTriangle size={48} className="text-[var(--color-alert)]" />
                </div>

                {/* Error Title */}
                <h1 className="text-3xl font-bold text-[var(--color-text)] mb-4">
                    Oops! Something Went Wrong
                </h1>

                {/* Error Message */}
                <p className="text-[var(--color-text)] mb-2">
                    {error.message || "An unexpected error occurred."}
                </p>

                {error.digest && (
                    <p className="text-xs text-gray-500 mb-6">
                        Error ID: {error.digest}
                    </p>
                )}

                {/* Description */}
                <p className="text-sm text-gray-600 mb-8">
                    We're sorry for the inconvenience. Please try again or return to the home page.
                </p>

                {/* Buttons */}
                <div className="flex flex-col gap-3">
                    <button
                        onClick={() => reset()}
                        className="w-full rounded-lg bg-[var(--color-main)] border-2 border-[var(--color-main)] px-5 py-3 text-center text-base font-medium text-white hover:bg-[var(--color-soft-white)] hover:text-[var(--color-main)] transition"
                    >
                        Try Again
                    </button>

                    <Link
                        href="/"
                        className="flex items-center justify-center gap-2 w-full rounded-lg border-2 border-[var(--color-main)] bg-[var(--color-soft-white)] px-5 py-3 text-center text-base font-medium text-[var(--color-main)] hover:bg-[var(--color-main)] hover:text-[var(--color-soft-white)] transition"
                    >
                        <Home size={18} />
                        Go to Home
                    </Link>
                </div>
            </div>
        </div>
    );
}
