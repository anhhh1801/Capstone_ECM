...
export default function CourseListTab({ courses, centerId, isManager, onUpdate }: Props) {
    ...
    const handleInvite = async (courseId: number) => {
        const email = prompt("Enter the email of the teacher to invite:");
        if (email) {
            try {
                await inviteTeacher(courseId, email);
                toast.success("Invitation sent!");
            } catch (e) { toast.error("Error sending invitation"); }
        }
    };
        ...
        <div>
            <div className="flex justify-between items-center mb-6">
                <h3 className="font-bold text-gray-700">
                    {isManager ? "All courses" : "Courses you are in charge of"}
                </h3>
                {isManager && (
                    <Link
                ...
                <table className="w-full text-left text-sm">
                    <thead className="bg-gray-50 font-semibold text-gray-700 uppercase text-xs">
                        <tr>
                            ...
                            {isManager && <th className="px-6 py-3">Teacher</th>}
                        ...
                        {courses.length === 0 ? (
                            <tr>
                                <td colSpan={5} className="p-6 text-center text-gray-500">
                                    {isManager ? "No courses yet" : "You haven't been assigned any classes"}
                                </td>
                            </tr>
                        ) : (
                            courses.map((course) => (
